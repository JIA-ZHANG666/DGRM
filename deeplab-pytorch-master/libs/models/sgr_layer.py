...
class LocalToSemantic(nn.Module):
    ...
    def __init__(self, input_feature_channels, visual_feature_channels,
        ...
        self.conv1 = nn.Conv2d(256, num_symbol_node,
                              kernel_size=1, stride=1)
        

        # 利用一层卷积将输入图片的channel由数目Dl转化为Dc, 即[？，Dc, H, W]
        self.conv2 = nn.Conv2d(256, 600,
                              kernel_size=1, stride=1)
...
class GRMLayer(nn.Module):
    ...
    def __init__(self, input_feature_channels,  visual_feature_channels, num_symbol_node,
        ...
        self.graph_reasoning1 = GraphConvolution(600,256)
        self.graph_reasoning2 = GraphConvolution(256,256)

        self.glore = GloRe(visual_feature_channels)
        self.final = nn.Sequential(nn.Conv2d(visual_feature_channels* 2, visual_feature_channels, kernel_size=1, bias=False))
...
class DualGCNHead(nn.Module):#0.6908,0.7055
    ...
    def forward(self, x):
        ...
        out = gc1 + F.interpolate(out1, size=(x.shape[2], x.shape[3]), mode="bilinear")
