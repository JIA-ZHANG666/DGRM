...
from libs.models import DeepLabV2_ResNet101_MSC_SGR, DeepLabV2_ResNet101_MSC_SGR_2, DeepLabV2_SGR
...
def train(config_path, cuda):
    ...
    assert (
        CONFIG.MODEL.NAME == "DeepLabV2_ResNet101_MSC_SGR"
    ...
    """model = DeepLabV2_SGR(n_classes=CONFIG.DATASET.N_CLASSES,
                           n_blocks=[3,4,23,3],
                           atrous_rates=[6,12,18,24],
                           input_feature_channels=CONFIG.DEEPLABV2.INPUT_FEATURE_CHANNELS,
                           visual_feature_channels=CONFIG.DEEPLABV2.VISUAL_FEATURE_CHANNELS,
                           **voc_data
                       )"""
    

    model = DeepLabV2_ResNet101_MSC_SGR(n_classes=CONFIG.DATASET.N_CLASSES,
                                    input_feature_channels=CONFIG.DEEPLABV2.INPUT_FEATURE_CHANNELS,
                                    visual_feature_channels=CONFIG.DEEPLABV2.VISUAL_FEATURE_CHANNELS,
                                    **voc_data
                                    #**coco_data
    ...
    optimizer = torch.optim.SGD(
        ...
        params=[
            ...
            {
                "params": get_params(model.module, key="2x"),
                "lr": 0.0001,
            ...
            {
                "params": get_params(model.module, key="3x"),
                "lr": 10 * CONFIG.SOLVER.LR,
                "weight_decay": CONFIG.SOLVER.WEIGHT_DECAY,
            },
            {
    ...
    #model.module.freeze_bn() # 关闭deeplabv2的bn
    ...
    for iteration in tqdm(
        ...
        loss = 0
        for i in range(CONFIG.SOLVER.ITER_SIZE):
            try:
            ...
            print('[Epoch={} iter={}] iter_loss is'.format(iteration,i), iter_loss)
            iter_loss.backward()
        ...
        print('Epoch {} :training loss {:.4f}'.format(iteration, loss))
